...
### D14 —— 可选服务的缺失**不该**由"整棵树加载失败"来兜底（I5 决策）

**背景**：`inject` 的语义是"这些服务必须在插件运行前就绪"。原设计把 `approval` 放进 `inject`，
理由是"没有审批器就无法安全执行 R2/R3" —— 理由没错，**手段错了**：

- 缺少审批服务的 profile 里，本插件的 `apply()` **根本不会执行** → 工具一个都没注册；
- 更糟的情况是那条 loader entry 加载失败，而**任何一条 entry 失败都会让整棵插件树加载不出来**
  （§5.2 坑 6 实测过）。**一个可选服务的缺失不该有让整个 GUI 起不来的权力。**

**决策**：`inject = ['tools']` —— 加载时永不阻塞，**执行时严格 fail-closed**：

| 情形 | 行为 |
|---|---|
| `tools` 缺失 | 插件不加载（没有它什么也做不了，失败是应该的） |
| `approval` 缺失 + R0/R1 动作 | **照常执行**（它们本来就不需要审批） |
| `approval` 缺失 + R2/R3 动作 | **一律拒绝**，明确说"审批服务不可用，按 fail-closed 拒绝"；`transport.apply` 调用次数为 0 |
| 任何情况 | 加载台账 + `vmprobe_status` 都显示"审批服务不可用"，**不静默** |
...
加密钥之后，伪造需要**密钥**而不只是写权限。四条设计：

1. **逐条记 `algo`**：老记录没有该字段（等价于 `sha256`），新记录是 `hmac-sha256`
   ⇒ **旧链照旧能验**，不需要迁移，也不假装历史被密钥保护过（`verifyChain` 会同时报
   `hmacCount` / `legacyCount`，混链时 `forgeryResistant: false`）。
2. **`algo` 与 `keyId` 进哈希正文**：否则把 `algo` 改成 `sha256` 再重算就能"一步降级"。
   `keyId` 是密钥的 sha256 前缀，**不是密钥**，可以安全进日志与审计。
3. **禁止降级**：链里一旦出现 hmac 记录，其后不得再出现 sha256 记录（报 `downgradeAt`）。
4. **换密钥 ≠ 被篡改**：`keyId` 不符时报 `keyMismatchAt` 并说明"无法校验，这不等于被篡改" ——
   把两种完全不同的处置混成一个错误，是运维事故的种子。

**诚实边界（必须写清楚）**：
- **整段重写仍无法由链自身识别**。攻击者若有写权限但没有密钥，可以把**整条链**重算成无密钥格式，
  得到一个自洽的"旧格式链" —— 链里不会留下任何 hmac 痕迹，因此无从分辨。
  为此提供 `requireHmac: true`：调用方**明确知道本机启用过密钥**时，可以要求"链里必须有 hmac 记录"，
  此时整段重写会被报出来（`unkeyedEntirely`）。默认不开，因为引擎无法区分
  "这是启用密钥之前的历史"与"历史被整段重写过"，不该替用户下这个判断。
- **密钥文件在同目录时不构成对"能读该目录"的攻击者的保护**。想要更强就把 `auditKeyFile`
  指到别处，或用 `auditKey` 从凭据库注入。这条写在 README 而不是藏起来。

**密钥来源**（优先级从高到低）：`auditHmac: false`（明确不用）→ `auditKey`（显式字符串）
→ 密钥文件（`auditKeyFile`，默认 `<storageDir>/audit-hmac.key`，不存在则**自动生成** 32 字节随机值，0600）。
密钥文件读写失败时**退化为无密钥并告警**，绝不因为一个密钥文件让插件加载失败。
实际生效情况在启动时写进审计（`config.effective.auditHmac` / `auditKeyId` / `auditKeySource`）——
事后能回答"当时的链到底抗不抗伪造"。

### D16 —— 运行日志与审计**分工**，不是同一份东西的两种写法

| | 审计 `logs/audit.jsonl` | 运行日志 `logs/run.jsonl` |
|---|---|---|
| 定位 | **证据**：带哈希链、逐条可校验、面向事后取证 | **可读流**：带 `level`、面向人看与 `grep` |
| 是否入链 | 是（HMAC 可选） | **否**（刻意不入链：日志格式变更不该破坏证据链） |
| 字段 | `ts/prev/hash/algo/keyId/...` | `ts/level/event/...`（**不含** `prev`/`hash`） |
| 轮转 | 多代（`audit.NNNN.jsonl`），跨文件链可校验 | 只留一代（`run.0001.jsonl`） |

级别由事件名推导（`*.failed`/`*.blocked`/`*.stale` → error；`*.skipped`/`cancelled`/`rotate` → warn；
其余 info），判据是**"这件事是否需要人注意"**，而不是事件名长短。

⚠️ 这里踩过一个坑：`maxRunLogBytes` 忘了给默认值，而 `size <= undefined` 恒为 false
⇒ **每写一条就轮转一次**，症状是"运行日志永远只有一行"，看起来像功能没生效。
已有专门的回归测试盯着（`packages/plugin-host/test/runlog.test.js`）。
...
## 11. 归档与迁移（需求 2.3）✅ **已实现（M3）**

### 11.1 归档格式（`.vmpz`，实为**标准 zip**）

```
vm-backup-20260914.vmpz
├── manifest.json          schema, createdAt, producer, redaction, includes,
│                          counts, 每文件 {path, bytes, sha256},
│                          **skipped[]**（跳过了什么、为什么）
├── data/
│   ├── targets.json       目标定义（无凭据，仅 authRef 引用；可按档位脱敏）
│   ├── facts/*.json       最近画像
│   ├── reports/**         每日报告历史
│   ├── loads.jsonl        加载台账
│   ├── logs/*.jsonl       审计链（需 --include-audit）
│   └── runs/*.log         运行记录（需 --include-runs）
└── secrets.enc.json       仅 --include-secrets：scrypt+AES-256-GCM 密文
```

**为什么是标准 zip**：归档要能被**别的工具**打开（Windows 资源管理器、`unzip`、另一台机器）。
因此没有自定义 tar，而是**用 Node 自带的 `zlib` 自己拼 ZIP 容器**（`packages/core/src/zip.js`，
约 200 行）—— 不为一个导出功能引入依赖树。
该实现用**外部实现交叉验证**：导出的归档能被 PowerShell 的 `Expand-Archive` 正常解开
（`tools/checks/check-archive.mjs`，这条验证很关键：否则"只有我自己能读"的格式也测试通过）。

### 11.2 兼容性与迁移

- `schema: 'vmprobe/archive/1'`；导入时**版本高于本机 → 拒绝**（明确说"请升级主控端"，
  旧程序读新格式只会误解内容）；低于 → 走注册的迁移函数（当前只有 v1）。
- **逐条 sha256 绑定**：清单里每个文件都记 `sha256` 与字节数，导入前一一核对；
  不符即拒绝 —— 归档是迁移载体，"尽力而为地导入半个"比拒绝更危险。
- **清单自证"没带走什么"**：`skipped[]` 逐条记下被排除的路径与原因，
  且校验时要求"声明跳过的东西确实不在归档里"（清单与内容矛盾即拒绝）。
- 跨平台归一化：归档内路径统一 `/`；时间戳统一 UTC RFC3339。

### 11.3 脱敏策略（导出时可选档位）

| 档 | 行为 |
|---|---|
| `none` | 原样导出（**仅限本机自用**；归档里会带真实 IP 与主机名） |
| `minimal` | 打码 IP 与主机名（`10.0.0.5` → `10.0.*.*`） |
| `standard`（默认） | 同 `minimal`，并把 `tags` 清空（标签常含"prod"这类环境语义） |

**`authRef` 引用名不打码**：它只是"引用名"，不是凭据；打码它反而会让迁移后无从对应。

### 11.4 凭据与密钥为何不随归档迁移

**决策**：归档**默认不含任何密码、私钥、口令**，只含 `authRef` 引用。两条硬规则：

1. **审计密钥（`audit-hmac.key`）永不入档** —— 它不是"配置"，而是"抗伪造的根"。
   它一旦随归档流出去，拿到归档的人就能伪造那份审计链，HMAC 等于白做。
   这条是**硬编码排除**（没有开关），并有测试盯着。
2. **私钥（`keys/`）默认不入档**；要带就必须 `--include-secrets` **且必须加密** ——
   "带秘密但不加密"这个组合**不存在**（代码里直接拒绝），口令只能从 TTY 或环境变量给，
   **不接受命令行参数**（那会进 shell 历史与 `ps` 输出）。

**迁移后的体验**：导入前先给差异预览（新增 / 一致 / 冲突 / **凭据缺口**），
导入时默认**不覆盖**本地已有且不同的文件（要覆盖得显式 `--overwrite`，且**先备份**为 `.vmpz-bak`）。
凭据缺口会逐个列出来，用掩码 CLI 补录即可。
