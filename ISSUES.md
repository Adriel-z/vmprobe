...
- 唯一待用户决策项仍是 **I5**（`approval` 是否保持必需注入）。

---

*报告结束。六轮合计：**可复现并修复的缺陷 43 项**（16 + 3 协从端 + 5 结构性 + 3 修复中发现 + 4 M1 实现中发现 + 3 心跳轮 + 5 M2 轮 + 2 取消轮 + 1 发布轮 + 1 测试台保真度），
另有 **5 项测试侧误判**与 **2 项测试台缺陷**（§11.2 / §11.3 / §12.3 / §12.6 / §12.7）单独列出，不计入产品缺陷数。
其中 **3 项只有真机启动才能发现**（定时器静默未启动、凭据文件致 DSH 无法启动、JSON schema 子集不兼容），
**1 项在写代码时被自己拦住**（`timeoutMs: null`，§12.2），
**1 项由"绿测试"掩盖**（取消被报成成功，§12.6），
**1 项只有真机发布才会暴露**（令牌被写进仓库工作区，§12.7）——
这几项都属于"一处出错就让整棵插件树加载不出来"或"看起来成功其实没做到"的致命形态，
因此现在有三道防线：**环境预检**（`doctor`）、**用宿主校验器做的契约检查**，
以及一条测试纪律：**"没发生"类断言必须跨过"本该发生的时刻"**。
剩下的是需要你决策的 1 项（I5）、受环境阻塞的 1 项（客户端 UI）与 M3 主线（归档与迁移）。*
