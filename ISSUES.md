...
*报告结束。六轮合计：**可复现并修复的缺陷 41 项**（16 + 3 协从端 + 5 结构性 + 3 修复中发现 + 4 M1 实现中发现 + 3 心跳轮 + 5 M2 轮 + 2 取消轮），
另有 **5 项测试侧误判**与 **1 项测试台缺陷**（§11.2 / §11.3 / §12.3 / §12.6）单独列出，不计入产品缺陷数。
...
因此现在有两道防线：**环境预检**（`doctor`）与**用宿主校验器做的契约检查**，
外加一条测试纪律：**"没发生"类断言必须跨过"本该发生的时刻"**。
